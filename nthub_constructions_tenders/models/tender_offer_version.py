...
class TenderOfferVersion(models.Model):
    ...
    customer_id = fields.Many2one('res.partner', string=_('Customer'), required=False)
    ...
    version_line_ids = fields.One2many('tender.offer.version.line', 'version_id', string=_('Version Lines'))
    ...
    state = fields.Selection([('draft', 'Draft'), ('confirm', 'Confirm')], tracking=True, default='draft', string=_('Status'))
    ...
    def action_update_unit_prices(self):
        """
        لكل سطر في الإصدار، يجيب إجمالي البريك داون المرتبط به
        ويقسمه على الكمية عشان يطلع سعر الوحدة تلقائياً.
        """
        self.ensure_one()
        updated = 0
        for line in self.version_line_ids:
            if not line.template_id:
                continue
            template = line.template_id
            # استخدم الإجمالي مع هامش الربح لو موجود، وإلا التكلفة الأساسية
            total = template.jobcost_total
            if line.qty and line.qty != 0:
                line.unit_price = total / line.qty
            else:
                line.unit_price = total
            updated += 1

        if updated == 0:
            raise UserError(_('No items linked to a breakdown. Link templates first.'))
...
class TenderOfferVersionLine(models.Model):
    ...
    unit_price = fields.Float(string=_('Price / Unit'))
