...
class TopSheet(models.Model):
    ...
    total_tender_cost = fields.Float(related="offer_id.total_lines_cost", string=_('Direct Cost'))
    indirect_cost_percentage = fields.Float(string=_('Indirect Cost Percentage'),
                                            compute='_compute_indirect_cost_percentage')
    # Indirect Cost = إجمالي الأوفر هيد بتاع العرض (مش المصاريف الإضافية)
    total_indirect_cost = fields.Float(related="offer_id.total_overhead_amount", string=_('Indirect Cost'), )
    sub_total = fields.Float(related="offer_id.total_lines_cost", string=_('Total Cost'))
