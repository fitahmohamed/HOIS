...
class ProjectCompletionRequest(models.Model):
    ...
    # ملحوظة: الـ @api.depends كانت ناقصة فالإجماليات كانت بتفضل صفر/قديمة
    # ومش بتتحدث لما الكميات أو النسب تتغير في السطور
    @api.depends('line_ids.total_amount', 'line_ids.quantity', 'line_ids.price_unit')
    ...
    def create(self,  vals_list):
        ...
        res = super(ProjectCompletionRequest, self).create(vals_list)
        return res
    ...
    def action_confirm(self):
        ...
        rejected_lines = self.line_ids.filtered(lambda x: x.state in (' ', 'reject') or x.amount == 0 or x.percentage == 0)
        if rejected_lines:
    ...
    def create_completion_invoice(self):
        ...
        for rec in self:
            ...
            customer_compilation_journal_account_id = int(self.env['ir.config_parameter'].sudo().get_param(
                'nthub_constructions.customer_compilation_journal_account_id'))
            customer_compilation_request_account_id = int(self.env['ir.config_parameter'].sudo().get_param(
                'nthub_constructions.customer_compilation_request_account_id'))

            if customer_compilation_journal_account_id and customer_compilation_request_account_id:
                invoice_vals = {
                    'partner_id': rec.contract_id.partner_id.id,
                    'invoice_date': rec.date,
                    'journal_id': customer_compilation_journal_account_id,
                ...
                for line in rec.line_ids:
                    ...
                    #     analytic_dist = {rec.project_id.analytic_account_id.id: 100}
                    invoice_line_vals = {
                        ...
                        'account_id': customer_compilation_request_account_id,
                    }
                    invoice_vals['invoice_line_ids'].append((0, 0, invoice_line_vals))
                    if rec.allowance_line_ids:
...
class ProjectEngineerTechnicalLine(models.Model):
    ...
    def _calc_qty(self):
        """تحديد سقف الكمية بالمتبقي من العقد.

        الكود القديم كان بيعد سطور المستخلص الحالي نفسه ضمن الكميات المحجوزة
        (لأنه بيدور على كل مستخلصات المشروع في draft/processing من غير ما
        يستثني المستخلص المفتوح) — فأي كمية تتكتب تاني كانت بتتصفّر.
        هنا بنستثني المستخلص الحالي وبنحسب على مستوى العقد مش المشروع."""
        # مهم: الميثود دي تحذير فقط — عمرها ما تعدّل أو تصفّر الكمية اللي
        # المستخدم كتبها. التصفير التلقائي القديم كان بيمسح الإدخال بسبب
        # بيانات متبقي قديمة/متضخمة. الفحص الملزم موجود في constraint
        # كمية المنفذ على بند العقد وقت الاعتماد.
        for rec in self:
            if not rec.item_id or not rec.completion_request_id or rec.quantity <= 0:
                continue
            ...
            if not contract_lines:
                continue
            # متبقي حي من مصدره مباشرة (مش الحقل المخزن اللي ممكن يكون قديم)
            ...
            if rec.quantity > remaining:
                return {'warning': {
                    ...
                    'message': _('البند "%s": الكمية المدخلة %.2f أكبر من المتبقي من العقد %.2f '
